...
r
        ...
        resultDiv.innerHTML = `<h2 style="font-family: 'Raleway', sans-serif; font-weight: 600; font-size: 18px; padding: 13px 23px 13px 24px; ">${qIndex + 1}. ${question.question}</h3>
            ${question.options.map((option, index) => `
                <div style="color: ${option === correctAnswer ? 'green' : (option === userAnswer ? 'red' : 'black')}; font-family: 'Raleway', sans-serif; font-weight: 200; ">
                    ${option}
                </div>
            `).join('')}`;
